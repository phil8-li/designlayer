...
ships with it: React, Sonner, Motion and motion-panels are compiled into
`dist/designlayer.js`, Sonner's stylesheet and Lucide's icon paths are copied
...
The editor's glyphs come from the design foundations kit and
[Lucide](https://lucide.dev) (ISC), vendored as path data by
`tools/build-icons.mjs` rather than imported, because the overlay bundle takes
